Version =20
VersionRequired =20
Begin Form
    AllowFilters = NotDefault
    RecordSelectors = NotDefault
    MaxButton = NotDefault
    MinButton = NotDefault
    AutoCenter = NotDefault
    NavigationButtons = NotDefault
    AllowDeletions = NotDefault
    CloseButton = NotDefault
    DividingLines = NotDefault
    AllowAdditions = NotDefault
    DefaultView =0
    ScrollBars =0
    ViewsAllowed =1
    PictureAlignment =2
    DatasheetGridlinesBehavior =3
    Cycle =1
    GridY =10
    Width =14173
    DatasheetFontHeight =11
    ItemSuffix =96
    Left =5112
    Top =1704
    Right =19188
    Bottom =8136
    RecSrcDt = Begin
        0x5c7a48f85bd8e540
    End
    Caption ="Demo C_LabelsInfo"
    OnClose ="[Event Procedure]"
    DatasheetFontName ="Calibri"
    AllowDatasheetView =0
    FilterOnLoad =0
    OrderByOnLoad =0
    OrderByOnLoad =0
    ShowPageMargins =0
    DisplayOnSharePointSite =1
    AllowLayoutView =0
    DatasheetAlternateBackColor =15921906
    DatasheetGridlinesColor12 =0
    FitToScreen =1
    DatasheetBackThemeColorIndex =1
    BorderThemeColorIndex =3
    ThemeFontIndex =1
    ForeThemeColorIndex =0
    AlternateBackThemeColorIndex =1
    AlternateBackShade =95.0
    Begin
        Begin Label
            BackStyle =0
            FontSize =11
            FontName ="Calibri"
            ThemeFontIndex =1
            BackThemeColorIndex =1
            BorderThemeColorIndex =0
            BorderTint =50.0
            ForeThemeColorIndex =0
            ForeTint =60.0
            GridlineThemeColorIndex =1
            GridlineShade =65.0
        End
        Begin Rectangle
            SpecialEffect =3
            BackStyle =0
            BorderLineStyle =0
            Width =850
            Height =850
            BackThemeColorIndex =1
            BorderThemeColorIndex =1
            BorderShade =65.0
            GridlineThemeColorIndex =1
            GridlineShade =65.0
        End
        Begin Line
            BorderLineStyle =0
            Width =1701
            BorderThemeColorIndex =0
            GridlineThemeColorIndex =1
            GridlineShade =65.0
        End
        Begin Image
            BackStyle =0
            OldBorderStyle =0
            BorderLineStyle =0
            SizeMode =3
            PictureAlignment =2
            Width =1701
            Height =1701
            BackThemeColorIndex =1
            BorderThemeColorIndex =1
            BorderShade =65.0
            GridlineThemeColorIndex =1
            GridlineShade =65.0
        End
        Begin CommandButton
            Width =1701
            Height =283
            FontSize =11
            FontWeight =400
            FontName ="Calibri"
            ForeThemeColorIndex =0
            ForeTint =75.0
            GridlineThemeColorIndex =1
            GridlineShade =65.0
            UseTheme =1
            Shape =1
            Gradient =12
            BackThemeColorIndex =4
            BackTint =60.0
            BorderLineStyle =0
            BorderThemeColorIndex =4
            BorderTint =60.0
            ThemeFontIndex =1
            HoverThemeColorIndex =4
            HoverTint =40.0
            PressedThemeColorIndex =4
            PressedShade =75.0
            HoverForeThemeColorIndex =0
            HoverForeTint =75.0
            PressedForeThemeColorIndex =0
            PressedForeTint =75.0
        End
        Begin OptionButton
            BorderLineStyle =0
            LabelX =230
            LabelY =-30
            BorderThemeColorIndex =1
            BorderShade =65.0
            GridlineThemeColorIndex =1
            GridlineShade =65.0
        End
        Begin CheckBox
            BorderLineStyle =0
            LabelX =230
            LabelY =-30
            BorderThemeColorIndex =1
            BorderShade =65.0
            GridlineThemeColorIndex =1
            GridlineShade =65.0
        End
        Begin BoundObjectFrame
            AddColon = NotDefault
            SizeMode =3
            SpecialEffect =2
            BorderLineStyle =0
            Width =4536
            Height =2835
            LabelX =-1701
            BackThemeColorIndex =1
            BorderThemeColorIndex =1
            BorderShade =65.0
            GridlineThemeColorIndex =1
            GridlineShade =65.0
        End
        Begin TextBox
            AddColon = NotDefault
            FELineBreak = NotDefault
            BorderLineStyle =0
            Width =1701
            LabelX =-1701
            FontSize =11
            FontName ="Calibri"
            AsianLineBreak =1
            BackThemeColorIndex =1
            BorderThemeColorIndex =1
            BorderShade =65.0
            ThemeFontIndex =1
            ForeThemeColorIndex =0
            ForeTint =75.0
            GridlineThemeColorIndex =1
            GridlineShade =65.0
        End
        Begin ListBox
            BorderLineStyle =0
            Width =1701
            Height =1417
            LabelX =-1701
            FontSize =11
            FontName ="Calibri"
            AllowValueListEdits =1
            InheritValueList =1
            ThemeFontIndex =1
            BackThemeColorIndex =1
            BorderThemeColorIndex =1
            BorderShade =65.0
            ForeThemeColorIndex =0
            ForeTint =75.0
            GridlineThemeColorIndex =1
            GridlineShade =65.0
        End
        Begin ComboBox
            AddColon = NotDefault
            BorderLineStyle =0
            Width =1701
            LabelX =-1701
            FontSize =11
            FontName ="Calibri"
            AllowValueListEdits =1
            InheritValueList =1
            ThemeFontIndex =1
            BackThemeColorIndex =1
            BorderThemeColorIndex =1
            BorderShade =65.0
            ForeThemeColorIndex =2
            ForeShade =50.0
            GridlineThemeColorIndex =1
            GridlineShade =65.0
        End
        Begin Subform
            BorderLineStyle =0
            Width =1701
            Height =1701
            BorderThemeColorIndex =1
            GridlineThemeColorIndex =1
            GridlineShade =65.0
            BorderShade =65.0
            ShowPageHeaderAndPageFooter =1
        End
        Begin ToggleButton
            Width =283
            Height =283
            ForeThemeColorIndex =0
            ForeTint =75.0
            GridlineThemeColorIndex =1
            GridlineShade =65.0
            UseTheme =1
            Shape =2
            Bevel =1
            BackColor =-1
            BackThemeColorIndex =4
            BackTint =60.0
            OldBorderStyle =0
            BorderLineStyle =0
            BorderColor =-1
            BorderThemeColorIndex =4
            BorderTint =60.0
            ThemeFontIndex =1
            HoverThemeColorIndex =4
            HoverTint =40.0
            PressedThemeColorIndex =4
            PressedShade =75.0
            HoverForeThemeColorIndex =0
            HoverForeTint =75.0
            PressedForeThemeColorIndex =1
        End
        Begin FormHeader
            Height =1023
            Name ="EntêteFormulaire"
            AlternateBackThemeColorIndex =1
            AlternateBackShade =95.0
            BackThemeColorIndex =3
            BackShade =25.0
            Begin
                Begin Label
                    OverlapFlags =85
                    Left =3968
                    Top =113
                    Width =3915
                    Height =480
                    FontSize =18
                    BorderColor =8355711
                    Name ="Étiquette64"
                    Caption ="Démo Classe C_LabelsInfo"
                    GridlineColor =10921638
                    LayoutCachedLeft =3968
                    LayoutCachedTop =113
                    LayoutCachedWidth =7883
                    LayoutCachedHeight =593
                    ForeThemeColorIndex =9
                End
            End
        End
        Begin Section
            CanGrow = NotDefault
            Height =5952
            Name ="Détail"
            AlternateBackThemeColorIndex =1
            AlternateBackShade =95.0
            BackThemeColorIndex =1
            Begin
                Begin Label
                    Visible = NotDefault
                    OverlapFlags =93
                    Left =170
                    Top =113
                    Width =7935
                    Height =285
                    FontSize =10
                    BorderColor =8355711
                    Name ="lbl_TexteInfo"
                    Caption =" "
                    FontName ="Verdana"
                    GridlineColor =10921638
                    LayoutCachedLeft =170
                    LayoutCachedTop =113
                    LayoutCachedWidth =8105
                    LayoutCachedHeight =398
                    ThemeFontIndex =-1
                    ForeTint =100.0
                End
                Begin Label
                    Visible = NotDefault
                    FontItalic = NotDefault
                    OldBorderStyle =1
                    BorderWidth =1
                    OverlapFlags =95
                    TextFontCharSet =2
                    TextFontFamily =18
                    Left =170
                    Top =396
                    Width =219
                    Height =228
                    FontSize =10
                    FontWeight =700
                    BorderColor =8355711
                    Name ="lbl_BarreInfo"
                    Caption ="a"
                    FontName ="Webdings"
                    GridlineColor =10921638
                    LayoutCachedLeft =170
                    LayoutCachedTop =396
                    LayoutCachedWidth =389
                    LayoutCachedHeight =624
                    ThemeFontIndex =-1
                    BackThemeColorIndex =7
                    BackTint =20.0
                    ForeThemeColorIndex =9
                    ForeTint =100.0
                    ForeShade =75.0
                End
                Begin CommandButton
                    TabStop = NotDefault
                    OverlapFlags =85
                    Left =1133
                    Top =3401
                    Width =2265
                    Height =453
                    ForeColor =4210752
                    Name ="cmdTestCls"
                    Caption ="Test dans une class"
                    OnClick ="[Event Procedure]"
                    GridlineColor =10921638

                    LayoutCachedLeft =1133
                    LayoutCachedTop =3401
                    LayoutCachedWidth =3398
                    LayoutCachedHeight =3854
                    BackColor =14461583
                    BorderColor =14461583
                    HoverColor =15189940
                    PressedColor =9917743
                    HoverForeColor =4210752
                    PressedForeColor =4210752
                    WebImagePaddingLeft =3
                    WebImagePaddingTop =3
                    WebImagePaddingRight =2
                    WebImagePaddingBottom =2
                    Overlaps =1
                End
                Begin CommandButton
                    TabStop = NotDefault
                    OverlapFlags =85
                    Left =1133
                    Top =4138
                    Width =2256
                    Height =448
                    TabIndex =1
                    ForeColor =4210752
                    Name ="cmdTestFrm"
                    Caption ="Test dans le form"
                    OnClick ="[Event Procedure]"
                    GridlineColor =10921638

                    LayoutCachedLeft =1133
                    LayoutCachedTop =4138
                    LayoutCachedWidth =3389
                    LayoutCachedHeight =4586
                    BackColor =14461583
                    BorderColor =14461583
                    HoverColor =15189940
                    PressedColor =9917743
                    HoverForeColor =4210752
                    PressedForeColor =4210752
                    WebImagePaddingLeft =3
                    WebImagePaddingTop =3
                    WebImagePaddingRight =2
                    WebImagePaddingBottom =2
                    Overlaps =1
                End
                Begin ComboBox
                    TabStop = NotDefault
                    RowSourceTypeInt =1
                    OverlapFlags =93
                    TextFontCharSet =2
                    TextFontFamily =18
                    IMESentenceMode =3
                    ListWidth =1440
                    Left =4762
                    Top =3514
                    Width =1116
                    Height =315
                    TabIndex =2
                    BorderColor =10921638
                    ForeColor =3484194
                    Name ="zlCarB1"
                    RowSourceType ="Value List"
                    RowSource ="\"a\";\"r\";\"s\";\"w\";\"E\";\"¯\";\"Ù\""
                    ColumnWidths ="1440"
                    DefaultValue ="\"a\""
                    FontName ="Webdings"
                    GridlineColor =10921638

                    LayoutCachedLeft =4762
                    LayoutCachedTop =3514
                    LayoutCachedWidth =5878
                    LayoutCachedHeight =3829
                    ThemeFontIndex =-1
                    Begin
                        Begin Label
                            OverlapFlags =93
                            Left =4762
                            Top =3174
                            Width =1692
                            Height =300
                            BorderColor =8355711
                            ForeColor =6710886
                            Name ="lbl_zlCarB1"
                            Caption ="Caractère :"
                            GridlineColor =10921638
                            LayoutCachedLeft =4762
                            LayoutCachedTop =3174
                            LayoutCachedWidth =6454
                            LayoutCachedHeight =3474
                        End
                    End
                End
                Begin TextBox
                    OverlapFlags =223
                    IMESentenceMode =3
                    Left =4762
                    Top =2721
                    Width =1131
                    Height =315
                    TabIndex =4
                    BorderColor =10921638
                    ForeColor =4210752
                    Name ="txtBoucle"
                    DefaultValue ="20"
                    GridlineColor =10921638

                    LayoutCachedLeft =4762
                    LayoutCachedTop =2721
                    LayoutCachedWidth =5893
                    LayoutCachedHeight =3036
                    Begin
                        Begin Label
                            OverlapFlags =93
                            Left =4762
                            Top =2437
                            Width =1524
                            Height =300
                            BorderColor =8355711
                            ForeColor =6710886
                            Name ="lbl_txtBoucle"
                            Caption ="Nb de boucle :"
                            GridlineColor =10921638
                            LayoutCachedLeft =4762
                            LayoutCachedTop =2437
                            LayoutCachedWidth =6286
                            LayoutCachedHeight =2737
                        End
                    End
                End
                Begin CommandButton
                    TabStop = NotDefault
                    OverlapFlags =85
                    Left =13436
                    Top =5102
                    Width =576
                    Height =576
                    TabIndex =6
                    ForeColor =4210752
                    Name ="cmdCloseFrm"
                    OnClick ="[Event Procedure]"
                    ControlTipText ="Fermer formulaire"
                    GridlineColor =10921638
                    ImageData = Begin
                        0x2800000010000000100000000100200000000000000000000000000000000000 ,
                        0x000000000000000000000000000000000000000082c2ea0982c2ea4b82c2ea90 ,
                        0x82c2eade00000000000000000000000000000000000000000000000000000000 ,
                        0x000000000000000082c2ea2182c2ea7582c2eab782c2eaf982c2eaff82c2eaff ,
                        0x82c2eaff00000000000000000000000000000000000000000000000000000000 ,
                        0x000000000000000082c2eaff82c2eaff82c2eaff82c2eaff82c2eaff82c2eaff ,
                        0x82c2eaff82c2eaff82c2eaff82c2eaff82c2eaff000000000000000000000000 ,
                        0x000000000000000082c2eaff82c2eaff82c2eaff82c2eaff82c2eaff82c2eaff ,
                        0x82c2eaffffffffffffffffffffffffff82c2eaff000000000000000000000000 ,
                        0x000000000000000082c2eaff82c2eaff82c2eaff82c2eaff82c2eaff82c2eaff ,
                        0x82c2eaffffffffffffffffffffffffa500000000000000000000000000000000 ,
                        0x000000000000000082c2eaff82c2eaff82c2eaff82c2eaff82c2eaff82c2eaff ,
                        0x82c2eaffffffffffffffffc000000000b17d4a90b17d4affb17d4af0b17d4a36 ,
                        0x000000000000000082c2eaff82c2eaff82c2eaff82c2eaff82c2eaff82c2eaff ,
                        0x82c2eaffffffffedffffff30b17d4a87b17d4affb17d4af0b17d4a3600000000 ,
                        0x000000000000000082c2eaff82c2eaff82c2eaffffffffffd7ecf8ff82c2eaff ,
                        0x82c2eaffffffff30b17d4a81b17d4affb17d4affb17d4affb17d4affb17d4aff ,
                        0xb17d4affb17d4aff82c2eaff82c2eaff82c2eaffdceef9ffc4e2f5ff82c2eaff ,
                        0x82c2eaffffffff27b17d4a7eb17d4affb17d4affb17d4affb17d4affb17d4aff ,
                        0xb17d4affb17d4aff82c2eaff82c2eaff82c2eaff82c2eaff82c2eaff82c2eaff ,
                        0x82c2eaffffffffe4ffffff27b17d4a84b17d4affb17d4af0b17d4a3900000000 ,
                        0x000000000000000082c2eaff82c2eaff82c2eaff82c2eaff82c2eaff82c2eaff ,
                        0x82c2eaffffffffffffffffbd00000000b17d4a8db17d4affb17d4af0b17d4a39 ,
                        0x000000000000000082c2eaff82c2eaff82c2eaff82c2eaff82c2eaff82c2eaff ,
                        0x82c2eaffffffffffffffffffffffffa500000000000000000000000000000000 ,
                        0x000000000000000082c2eaff82c2eaff82c2eaff82c2eaff82c2eaff82c2eaff ,
                        0x82c2eaffffffffffffffffffffffffff82c2eaff000000000000000000000000 ,
                        0x000000000000000082c2eaff82c2eaff82c2eaff82c2eaff82c2eaff82c2eaff ,
                        0x82c2eaff82c2eaff82c2eaff82c2eaff82c2eaff000000000000000000000000 ,
                        0x000000000000000082c2ea2182c2ea6f82c2eab782c2eaf982c2eaff82c2eaff ,
                        0x82c2eaff00000000000000000000000000000000000000000000000000000000 ,
                        0x000000000000000000000000000000000000000082c2ea0982c2ea4e82c2ea96 ,
                        0x82c2eae400000000000000000000000000000000000000000000000000000000 ,
                        0x0000000000000000
                    End

                    LayoutCachedLeft =13436
                    LayoutCachedTop =5102
                    LayoutCachedWidth =14012
                    LayoutCachedHeight =5678
                    BackColor =14461583
                    BorderColor =14461583
                    HoverColor =15189940
                    PressedColor =9917743
                    HoverForeColor =4210752
                    PressedForeColor =4210752
                    WebImagePaddingLeft =3
                    WebImagePaddingTop =3
                    WebImagePaddingRight =2
                    WebImagePaddingBottom =2
                End
                Begin Label
                    Visible = NotDefault
                    OverlapFlags =95
                    Left =170
                    Top =623
                    Width =7935
                    Height =285
                    FontSize =10
                    BorderColor =8355711
                    Name ="lbl_TexteInfo2"
                    Caption =" "
                    FontName ="Verdana"
                    GridlineColor =10921638
                    LayoutCachedLeft =170
                    LayoutCachedTop =623
                    LayoutCachedWidth =8105
                    LayoutCachedHeight =908
                    ThemeFontIndex =-1
                    ForeTint =100.0
                End
                Begin Label
                    Visible = NotDefault
                    FontItalic = NotDefault
                    OldBorderStyle =1
                    BorderWidth =1
                    OverlapFlags =87
                    TextFontCharSet =2
                    TextFontFamily =18
                    Left =170
                    Top =907
                    Width =219
                    Height =228
                    FontSize =10
                    FontWeight =700
                    BorderColor =8355711
                    Name ="lbl_BarreInfo2"
                    Caption ="a"
                    FontName ="Webdings"
                    GridlineColor =10921638
                    LayoutCachedLeft =170
                    LayoutCachedTop =907
                    LayoutCachedWidth =389
                    LayoutCachedHeight =1135
                    ThemeFontIndex =-1
                    BackThemeColorIndex =7
                    BackTint =20.0
                    ForeThemeColorIndex =9
                    ForeTint =100.0
                    ForeShade =75.0
                End
                Begin ComboBox
                    TabStop = NotDefault
                    RowSourceTypeInt =1
                    OverlapFlags =93
                    TextFontCharSet =2
                    TextFontFamily =18
                    IMESentenceMode =3
                    ListWidth =1440
                    Left =6916
                    Top =5102
                    Width =1116
                    Height =315
                    TabIndex =3
                    BorderColor =10921638
                    ForeColor =3484194
                    Name ="zlCarB2"
                    RowSourceType ="Value List"
                    RowSource ="\"a\";\"r\";\"s\";\"w\";\"E\";\"¯\";\"Ù\""
                    ColumnWidths ="1441"
                    DefaultValue ="\"r\""
                    FontName ="Webdings"
                    GridlineColor =10921638

                    LayoutCachedLeft =6916
                    LayoutCachedTop =5102
                    LayoutCachedWidth =8032
                    LayoutCachedHeight =5417
                    ThemeFontIndex =-1
                    Begin
                        Begin Label
                            OverlapFlags =93
                            Left =6916
                            Top =4762
                            Width =1692
                            Height =300
                            BorderColor =8355711
                            ForeColor =6710886
                            Name ="lbl_zlCarB2"
                            Caption ="Caractère :"
                            GridlineColor =10921638
                            LayoutCachedLeft =6916
                            LayoutCachedTop =4762
                            LayoutCachedWidth =8608
                            LayoutCachedHeight =5062
                        End
                    End
                End
                Begin TextBox
                    OverlapFlags =223
                    IMESentenceMode =3
                    Left =4649
                    Top =4649
                    Width =1131
                    Height =315
                    TabIndex =5
                    BorderColor =10921638
                    ForeColor =4210752
                    Name ="txtBoucle2Dep"
                    ValidationText ="Max 70 pour ce form"
                    AfterUpdate ="[Event Procedure]"
                    DefaultValue ="10"
                    OnDirty ="[Event Procedure]"
                    GridlineColor =10921638

                    LayoutCachedLeft =4649
                    LayoutCachedTop =4649
                    LayoutCachedWidth =5780
                    LayoutCachedHeight =4964
                    Begin
                        Begin Label
                            OverlapFlags =93
                            Left =4649
                            Top =4365
                            Width =2268
                            Height =300
                            BorderColor =8355711
                            ForeColor =6710886
                            Name ="lbl_txtBoucle2Dep"
                            Caption ="Demarre à x de boucle 1"
                            GridlineColor =10921638
                            LayoutCachedLeft =4649
                            LayoutCachedTop =4365
                            LayoutCachedWidth =6917
                            LayoutCachedHeight =4665
                        End
                    End
                End
                Begin TextBox
                    OverlapFlags =223
                    IMESentenceMode =3
                    Left =4649
                    Top =5329
                    Width =1131
                    Height =315
                    TabIndex =7
                    BorderColor =10921638
                    ForeColor =4210752
                    Name ="txtBoucle2"
                    DefaultValue ="15"
                    GridlineColor =10921638

                    LayoutCachedLeft =4649
                    LayoutCachedTop =5329
                    LayoutCachedWidth =5780
                    LayoutCachedHeight =5644
                    Begin
                        Begin Label
                            OverlapFlags =93
                            Left =4649
                            Top =5045
                            Width =1524
                            Height =300
                            BorderColor =8355711
                            ForeColor =6710886
                            Name ="lbl_txtBoucle2"
                            Caption ="Nb de boucle :"
                            GridlineColor =10921638
                            LayoutCachedLeft =4649
                            LayoutCachedTop =5045
                            LayoutCachedWidth =6173
                            LayoutCachedHeight =5345
                        End
                    End
                End
                Begin CheckBox
                    TabStop = NotDefault
                    OverlapFlags =93
                    Left =7029
                    Top =2920
                    TabIndex =8
                    BorderColor =10921638
                    Name ="chkB1"
                    DefaultValue ="True"
                    GridlineColor =10921638

                    LayoutCachedLeft =7029
                    LayoutCachedTop =2920
                    LayoutCachedWidth =7289
                    LayoutCachedHeight =3160
                    Begin
                        Begin Label
                            OverlapFlags =255
                            Left =7259
                            Top =2890
                            Width =900
                            Height =300
                            BorderColor =8355711
                            ForeColor =6710886
                            Name ="lbl_chkB1"
                            Caption ="Barre 1"
                            GridlineColor =10921638
                            LayoutCachedLeft =7259
                            LayoutCachedTop =2890
                            LayoutCachedWidth =8159
                            LayoutCachedHeight =3190
                        End
                    End
                End
                Begin CheckBox
                    TabStop = NotDefault
                    OverlapFlags =93
                    Left =7029
                    Top =3366
                    TabIndex =9
                    BorderColor =10921638
                    Name ="chkT1"
                    DefaultValue ="True"
                    GridlineColor =10921638

                    LayoutCachedLeft =7029
                    LayoutCachedTop =3366
                    LayoutCachedWidth =7289
                    LayoutCachedHeight =3606
                    Begin
                        Begin Label
                            OverlapFlags =255
                            Left =7259
                            Top =3336
                            Width =900
                            Height =300
                            BorderColor =8355711
                            ForeColor =6710886
                            Name ="lbl_chkT1"
                            Caption ="Texte 1"
                            GridlineColor =10921638
                            LayoutCachedLeft =7259
                            LayoutCachedTop =3336
                            LayoutCachedWidth =8159
                            LayoutCachedHeight =3636
                        End
                    End
                End
                Begin CheckBox
                    TabStop = NotDefault
                    OverlapFlags =93
                    Left =9071
                    Top =4735
                    TabIndex =10
                    BorderColor =10921638
                    Name ="chkB2"
                    DefaultValue ="True"
                    GridlineColor =10921638

                    LayoutCachedLeft =9071
                    LayoutCachedTop =4735
                    LayoutCachedWidth =9331
                    LayoutCachedHeight =4975
                    Begin
                        Begin Label
                            OverlapFlags =255
                            Left =9301
                            Top =4705
                            Width =900
                            Height =300
                            BorderColor =8355711
                            ForeColor =6710886
                            Name ="lbl_chkB2"
                            Caption ="Barre 2"
                            GridlineColor =10921638
                            LayoutCachedLeft =9301
                            LayoutCachedTop =4705
                            LayoutCachedWidth =10201
                            LayoutCachedHeight =5005
                        End
                    End
                End
                Begin CheckBox
                    TabStop = NotDefault
                    OverlapFlags =93
                    Left =9071
                    Top =5155
                    TabIndex =11
                    BorderColor =10921638
                    Name ="chkT2"
                    DefaultValue ="True"
                    GridlineColor =10921638

                    LayoutCachedLeft =9071
                    LayoutCachedTop =5155
                    LayoutCachedWidth =9331
                    LayoutCachedHeight =5395
                    Begin
                        Begin Label
                            OverlapFlags =255
                            Left =9301
                            Top =5125
                            Width =900
                            Height =300
                            BorderColor =8355711
                            ForeColor =6710886
                            Name ="lbl_chkT2"
                            Caption ="Texte 2"
                            GridlineColor =10921638
                            LayoutCachedLeft =9301
                            LayoutCachedTop =5125
                            LayoutCachedWidth =10201
                            LayoutCachedHeight =5425
                        End
                    End
                End
                Begin Rectangle
                    OverlapFlags =255
                    Left =4535
                    Top =2267
                    Width =3912
                    Height =1699
                    BorderColor =10921638
                    Name ="Boîte89"
                    GridlineColor =10921638
                    LayoutCachedLeft =4535
                    LayoutCachedTop =2267
                    LayoutCachedWidth =8447
                    LayoutCachedHeight =3966
                End
                Begin Rectangle
                    OverlapFlags =255
                    Left =4535
                    Top =4308
                    Width =5784
                    Height =1411
                    BorderColor =10921638
                    Name ="Boîte90"
                    GridlineColor =10921638
                    LayoutCachedLeft =4535
                    LayoutCachedTop =4308
                    LayoutCachedWidth =10319
                    LayoutCachedHeight =5719
                End
                Begin Label
                    OverlapFlags =247
                    Left =4592
                    Top =1984
                    Width =720
                    Height =300
                    FontWeight =700
                    BorderColor =8355711
                    ForeColor =6710886
                    Name ="Étiquette91"
                    Caption ="Barre 1"
                    GridlineColor =10921638
                    LayoutCachedLeft =4592
                    LayoutCachedTop =1984
                    LayoutCachedWidth =5312
                    LayoutCachedHeight =2284
                End
                Begin Label
                    OverlapFlags =247
                    Left =4648
                    Top =4025
                    Width =720
                    Height =300
                    FontWeight =700
                    BorderColor =8355711
                    ForeColor =6710886
                    Name ="Étiquette92"
                    Caption ="Barre 2"
                    GridlineColor =10921638
                    LayoutCachedLeft =4648
                    LayoutCachedTop =4025
                    LayoutCachedWidth =5368
                    LayoutCachedHeight =4325
                End
                Begin Label
                    FontUnderline = NotDefault
                    OverlapFlags =85
                    Left =12415
                    Top =5272
                    Width =636
                    Height =312
                    BorderColor =8355711
                    Name ="lblDoc"
                    Caption ="Doc ..."
                    HyperlinkAddress ="https://github.com/meuslaur/MsAccess-ProgressBarre/wiki"
                    GridlineColor =10921638
                    LayoutCachedLeft =12415
                    LayoutCachedTop =5272
                    LayoutCachedWidth =13051
                    LayoutCachedHeight =5584
                    ForeThemeColorIndex =10
                    ForeTint =100.0
                End
                Begin Image
                    SizeMode =0
                    Left =12188
                    Top =5328
                    Width =192
                    Height =192
                    BorderColor =10921638
                    Name ="imgLblDoc"
                    Picture ="ic_Aide.png"
                    GridlineColor =10921638
                    ImageData = Begin
                        0x89504e470d0a1a0a0000000d49484452000000100000001008060000001ff3ff ,
                        0x6100000006624b4744000000000000f943bb7f000000097048597300000b1300 ,
                        0x000b1301009a9c1800000009767041670000001000000010005cc6adc3000002 ,
                        0xf94944415438cb6593cb6b9c6514c67fef65be4c663293cb64a663c624a498b4 ,
                        0x915034501a94d2228a141762a9086ebcac05372e15d78a0bff02bb100cdec085 ,
                        0x1b15b1246dbca0ad42ad99d4a44dc98dce64ee97cc37dff71e176924c60367f7 ,
                        0x3cbf730e9c47f1ff1a323632d33798396f6303534ed041bb76b75dbdbf10f89d ,
                        0x1b4011080fc4ea90d100934323c7df489f9cbb94187b6c58e2695ddf0b099b25 ,
                        0xf1b76f556bab3f7d53df5cf9d089bb01f847018f66271fff60fceccbcf78a3b3 ,
                        0xd627821f0ab5764037142c21bab8ec2abf7ef6fb6e7ee94d11f91108cd03737a ,
                        0x7874eadd89a75ebf68c7cf58df698c8644d4628da2e58704a291be8c4a0ce7b2 ,
                        0xd4b71e6e9777ae02650b9888d733979d3977d11b9b357ba190497abcffd23483 ,
                        0xf1084e845fd6aabcfd459efa5e80a4a6546ae6e973adc2fa8576adf49106fa93 ,
                        0x99b1e7fa1f3993ec8a45447876264d27745c5ebcc7d2ed32e7a753bc783a8b52 ,
                        0xa09422363edb933836f13c306c81647c68645627b2ca778208fc7dbfc9fccf9b ,
                        0x941b5d32c91ece9e48911b8a62b5c268858da788a7c74e70fbb7010b446d2c31 ,
                        0xe84c0fce09a10857964b844e48273dde796112a315d7ef56d16a1fa0adc58bf7 ,
                        0xf76945bf06101786ce399c8008382738275c3afd1033b9240bf95dbebd592462 ,
                        0xf601462b940b9d08be069a9d467913bf8113f94f0fc4227c77b3c05bf3b7fe35 ,
                        0x1aadb0e2e3377677051a1a28370b1bd73ac53b210a8250f003210821bfd3e08f ,
                        0x7bd507b783d1608d426adb52dfb9731d2868a0512b6c7c5dfa6b6143da55da5d ,
                        0x871fee6ff0cadc08af3e913b345d13555d2acb8b95caf6daa740f5e091aa7ebd ,
                        0x24d1dede2723a9e39100831398cec529363a5cc957f08c266e1d9dd5abddf5a5 ,
                        0x2f2f37abbb9f00b50380dfedb456fdf206bdda3f151fc8449517e3877c85c595 ,
                        0x0ab108c4820aadfcf7edf56b9fcf97b6d6de03368e660120edc51217fa73275f ,
                        0xebcb4d9ff206b271638c72cd62bbb1b9bc525afff3e356adf415b07590c8a300 ,
                        0x000f38668c9df07a63a3a06c77afb51504dd55601b681f16ff03d141689ccdfb ,
                        0x012a0000001d74455874436f6d6d656e74004372656174656420776974682054 ,
                        0x68652047494d50ef64256e00000025744558746372656174652d646174650032 ,
                        0x3030392d31312d31375432303a30363a35352d30373a303059a62abb00000025 ,
                        0x74455874646174653a63726561746500323031302d30312d31315431343a3239 ,
                        0x3a35392d30373a3030eb6c0cbd0000002574455874646174653a6d6f64696679 ,
                        0x00323031302d30312d31315431343a32393a35392d30373a30309a31b4010000 ,
                        0x0034744558744c6963656e736500687474703a2f2f6372656174697665636f6d ,
                        0x6d6f6e732e6f72672f6c6963656e7365732f47504c2f322e302f6c6a06a80000 ,
                        0x0025744558746d6f646966792d6461746500323030392d31312d31375432303a ,
                        0x30363a35352d30373a303006175c8f0000001874455874536f7572636500496e ,
                        0x7472696775652049636f6e20536574ae27a13f0000004674455874536f757263 ,
                        0x655f55524c00687474703a2f2f73696d706c65696e6e6f766174696f6e2e6e65 ,
                        0x742f696e6465782e7068703f706167653d323226736f757263653d322669643d ,
                        0x329541f9c60000000049454e44ae426082
                    End

                    LayoutCachedLeft =12188
                    LayoutCachedTop =5328
                    LayoutCachedWidth =12380
                    LayoutCachedHeight =5520
                    TabIndex =12
                End
            End
        End
        Begin FormFooter
            Height =0
            Name ="PiedFormulaire"
            AlternateBackThemeColorIndex =1
            AlternateBackShade =95.0
            BackThemeColorIndex =2
            BackTint =20.0
        End
    End
End
CodeBehindForm
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = True
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
'@Folder("Demo")
Option Compare Database
Option Explicit

'//---------Code Demo-----
    Private b1 As Access.Label
    Private t1 As Access.Label
    Private b2 As Access.Label
    Private t2 As Access.Label
    Private cDemo   As C_Demo
'//---------Code Demo-----

    Private cLabels  As C_LabelsInfo                                        '// Code C_LabelsInfo

Private Sub Form_Close()
    Set cDemo = Nothing
    Set cLabels = Nothing                                                   '// Code C_LabelsInfo
End Sub

Private Sub cmdCloseFrm_Click()
    DoCmd.Close
End Sub

Private Sub cmdTestCls_Click()

    InitLabels
    LanceDemo True

    Set cLabels = cDemo.GetClsLabelsInfo()                                  '// Code C_LabelsInfo
    cLabels.InitialiseLabels b1, t1, b2, t2                                 '// Code C_LabelsInfo
    cLabels.SetLengthBarre Me.zlCarB1, Me.txtBoucle                         '// Code C_LabelsInfo

    cDemo.BoucleTestCls (Me.txtBoucle)
    LanceDemo False

End Sub


Private Sub cmdTestFrm_Click()
    InitLabels
    LanceDemo True

    Set cLabels = New C_LabelsInfo                                          '// Code C_LabelsInfo
    cLabels.InitialiseLabels b1, t1, b2, t2                                 '// Code C_LabelsInfo
    cLabels.SetLengthBarre Me.zlCarB1, Me.txtBoucle                         '// Code C_LabelsInfo

    BoucleDemo Me.txtBoucle
    cLabels.HiddenCurrentLabels                                             '// Code C_LabelsInfo
    LanceDemo False

End Sub

Private Sub BoucleDemo(Boucle As Long)
    '//---------Demo-----
    Dim i As Long
    Dim lRand As Long
    Dim sAvance As String

    For i = 1 To Boucle
        lRand = Int((700 * Rnd) + 1)
        sAvance = "Boucle 1 : " & CStr(i)
        cLabels.UpdateBarre sAvance                                         '// Code C_LabelsInfo
        If (i = Me.txtBoucle2Dep) Then
            cLabels.SetLengthBarre Me.zlCarB2, Me.txtBoucle2, LabelNum2     '// Code C_LabelsInfo
            BoucleDemo2 Me.txtBoucle2
            cLabels.ReturnFirstBarre                                        '// Code C_LabelsInfo
        End If
        DoEvents                        '// Refresh labels.
        Sleep (lRand)
   Next i

End Sub

Private Sub BoucleDemo2(Boucle As Long)
    Dim i As Long
    Dim lRand As Long
    Dim sAvance As String

    For i = 1 To Boucle
        lRand = Int((700 * Rnd) + 1)
        sAvance = "Boucle 2 : " & CStr(i)
        cLabels.UpdateBarre sAvance                                         '// Code C_LabelsInfo
        DoEvents                        '// Refresh labels.
        Sleep (lRand)
   Next i
End Sub

'//---------Code Demo-----

Private Sub LanceDemo(Active As Boolean)


'    Me.lbl_TexteInfo.Visible = Active
'    Me.lbl_BarreInfo.Visible = Active
    DoCmd.Hourglass Active

    Me.txtBoucle.SetFocus
    Me.cmdCloseFrm.Enabled = Not Active
    Me.cmdTestCls.Enabled = Not Active
    Me.cmdTestFrm.Enabled = Not Active

    If (Not Active) Then
        Me.lbl_TexteInfo.Caption = "Fin"
        Me.lbl_BarreInfo.Caption = vbNullString
        ResetCls
    Else
        Set cDemo = New C_Demo
    End If

End Sub

Private Sub ResetCls()
    Set cDemo = Nothing
    Set cLabels = Nothing
End Sub

Private Sub txtBoucle2Dep_AfterUpdate()
    Me.txtBoucle2Dep = IIf(Me.txtBoucle2Dep > Me.txtBoucle, 0, Me.txtBoucle2Dep)
End Sub

Private Sub InitLabels()
    Set b1 = IIf(Me.chkB1, Me.lbl_BarreInfo, Nothing)
    Set t1 = IIf(Me.chkT1, Me.lbl_TexteInfo, Nothing)
    Set b2 = IIf(Me.chkB2, Me.lbl_BarreInfo2, Nothing)
    Set t2 = IIf(Me.chkT2, Me.lbl_TexteInfo2, Nothing)
End Sub
'//---------Code Demo-----
